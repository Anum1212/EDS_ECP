{{--*/ $inProcessVouchers = \App\Voucher::with('department.businessUnit.company')->whereHas('department.businessUnit.company', function($query) use ($employee){$query->where('companies.id', '=', $employee->department->businessUnit->company->id);})->whereIn('status', ['In Process'])->whereNull('is_travel_order')->count(); /*--}}
{{--*/ $inProcessTravelOrders = \App\Voucher::with('department.businessUnit.company')->whereHas('department.businessUnit.company', function($query) use ($employee){$query->where('companies.id', '=', $employee->department->businessUnit->company->id);})->whereIn('status', ['In Process'])->where('is_travel_order', '=', 1)->count(); /*--}}
{{--*/ $documentsTransitVouchers = \App\Voucher::with('department.businessUnit.company')->whereHas('department.businessUnit.company', function($query) use ($employee){$query->where('companies.id', '=', $employee->department->businessUnit->company->id);})->whereIn('status', ['Approved - Documents in transit'])->count(); /*--}}
