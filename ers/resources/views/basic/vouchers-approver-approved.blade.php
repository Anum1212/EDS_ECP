                ...
                <div class="col-10">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Vouchers Approved by You</h4>
                            ...
                            <div class="card-body card-dashboard table-responsive w-100 overflow-auto">
                                <table class="table table-striped table-bordered row-grouping">
                                    ...
                                    <tbody>
                                    @foreach($employee->vouchersApproved as $voucher)
                                        <tr>
                                            ...
                                            @if($employee->user_name == 'medicines-mother-admin')
                                            <td><small>PKR </small>{{number_format($voucher->totalAmountApproved($voucher->id), 0)}}</td>
                                    @endif
                                            @endforeach
                                    @foreach($employee->travelOrdersApproved as $voucher)
                                        <tr>
        ...
        $(document).ready(function(){
            $('.row-grouping').DataTable({
                ...
                "drawCallback": function ( settings ) {
                    ...
                    var numberOfColumns = api.columns(':visible').count();
                    var last=null;
                    ...
                    api.column(1, {page:'current'} ).data().each( function ( group, i ) {
                        if ( last !== group ) {
                            if(group == 'open'){
                                $(rows).eq( i ).before(
                                    '<tr class="group bg-success"><td colspan="'+numberOfColumns+'">'+group+'</td></tr>'
                                );
                            }
                            else{
                                $(rows).eq( i ).before(
