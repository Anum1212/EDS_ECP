<div class="card">
    <div class="card-header card-head-inverse bg-primary">
        <a data-action="collapse"><h4 class="card-title text-white">{{$category->category_name}} <span
                        class="badge badge bg-white text-secondary badge-pill mr-2"><small>PKR </small>{{number_format($voucher->categoryTotalAmount($voucher->id, $category->id), 0)}}</span>
            </h4></a>
                ...
                <table class="table table-white-space table-bordered display no-wrap table-middle">
                    ...
                    @foreach($voucher->categoryItems($voucher->id, $category->id) as $item)
                        <tr class="taskrow">
                            ...
                            <td>{{$item->evidence == 1? 'Manually Entered': 'Fetched From API'}}</td>
                            @if(count($voucher->voucherStatus) == 0)
                                <td><a href="{{URL::to('voucher/item/delete'.'/'.$item->id)}}" class="danger"><i
