<div class="card" id="{{$categoryName}}" data-parent="{{$parent}}">
    <div class="card-header card-head-inverse bg-secondary">
        <a data-action="collapse"><h4 class="card-title text-white">Mobile - Handset <span class="badge badge bg-white text-secondary badge-pill mr-2"></span></h4></a>
    ...
    <div class="card-content collapse" style="overflow-x: auto">
        <div class="card-body">
            <p class="danger"><strong>Don't forget to get the print out signed by Human Resource Department</strong></p>
            @if(isset($voucher))
                @foreach($voucher->categoryItems($voucher->id,$categoryID) as $item)
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-2 mb-2" hidden>
                            <label>Amount</label>
                            <input type="number" class="form-control" data-id='225' name="amount_{{$itemCount}}" id="amount_{{$itemCount}}" value="{{$item->amount}}">
                        </div>
                        <div class="form-group col-md-2 mb-2">
                            <label>Amount paid</label>
                            <input type="text" class="form-control" data-id='225' name="amount_paid_{{$itemCount}}" id="amount_paid_{{$itemCount}}" value="{{$item->amount_paid}}" required>
                        </div>
                        <div class="form-group col-md-2 mb-2">
