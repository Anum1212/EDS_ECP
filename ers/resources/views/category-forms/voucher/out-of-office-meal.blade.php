                ...
                @foreach($voucher->categoryItems($voucher->id,$categoryID) as $item)
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-2 mb-2">
                            <label>Date</label>
                            <input type="date" class="form-control meal-out-office-date" name="date_{{$itemCount}}" id="date_{{$itemCount}}" min="{{date('Y-m-d', strtotime(date('Y-m-d').' - 3 months'))}}" value="{{date('Y-m-d', strtotime($item->date_from))}}" required>
                        </div>
                        <div class="form-group col-md-5 mb-2">
                            <label>Meal Dates</label>
                            <input type="text" class="form-control mealOutOfLahore" value="{{$item->meal_dates}}" name="meal_dates_{{$itemCount}}" id="meal_dates_{{$itemCount}}" readonly required>
                        </div>
                        <div class="form-group col-md-2 mb-2">
                            <label>Description</label>
                            <select class="select2 form-control" name="description_{{$itemCount}}" required>
                                <option value="">Please Select</option>
                                <option {{$item->description == 'Lunch'?'selected':''}} value="Lunch">Lunch</option>
                                <option {{$item->description == 'Dinner'?'selected':''}}value="Dinner">Dinner</option>
                            </select>
                        </div>
                        <div class="form-group col-md-2 mb-2">
