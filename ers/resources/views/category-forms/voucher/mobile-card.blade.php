        ...
        <div class="card-body">
                ...
                @foreach($voucher->categoryItems($voucher->id,$categoryID) as $item)
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-2 mb-2">
                            <label>Date</label>
                            <input type="date" class="form-control" name="date_{{$itemCount}}" id="date_{{$itemCount}}" value="{{date('Y-m-d', strtotime($item->date_from))}}" required>
                        </div>
                        <div class="form-group col-md-2 mb-2">
                            <label>Month</label>
                            <select class="select2 form-control" name="month_{{$itemCount}}" required>
                                <option {{$item->month == 'January'?'selected':''}} value="January">January</option>
                                <option {{$item->month == 'February'?'selected':''}} value="February">February</option>
                                <option {{$item->month == 'March'?'selected':''}} value="March">March</option>
                                <option {{$item->month == 'April'?'selected':''}} value="April">April</option>
                                <option {{$item->month == 'May'?'selected':''}} value="May">May</option>
                                <option {{$item->month == 'June'?'selected':''}} value="June">June</option>
                                <option {{$item->month == 'July'?'selected':''}} value="July">July</option>
                                <option {{$item->month == 'August'?'selected':''}} value="August">August</option>
                                <option {{$item->month == 'September'?'selected':''}} value="September">September</option>
                                <option {{$item->month == 'October'?'selected':''}} value="October">October</option>
                                <option {{$item->month == 'November'?'selected':''}} value="November">November</option>
                                <option {{$item->month == 'December'?'selected':''}} value="December">December</option>
                            </select>
                        </div>
                        <div class="form-group col-md-2 mb-2">
                            <label>Description</label>
                            <input type="text" class="form-control" data-id='225' name="description_{{$itemCount}}" id="description_{{$itemCount}}" value="{{$item->description}}" required>
                        </div>
                        <div class="form-group col-md-2 mb-2" hidden>
                            <label>Amount (PKR)</label>
                            <input type="text" class="form-control" value="{{$item->amount}}" name="amount_{{$itemCount}}" id="amount_{{$itemCount}}">
                        </div>
                        <div class="form-group col-md-2 mb-2">
                            <label>Amount paid</label>
                            <input type="text" class="form-control" value="{{$item->amount_paid}}" name="amount_paid_{{$itemCount}}" id="amount_paid_{{$itemCount}}" required>
                        </div>
                        <div class="form-group col-md-2 mb-2">
            ...
            @else
                <div class="form-row">
                    ...
                    <div class="form-group col-md-2 mb-2">
                        <label>Description</label>
                        <input type="text" class="form-control" data-id='225' name="description_{{$itemCount}}" id="description_{{$itemCount}}" required>
                    </div>
                    <div class="form-group col-md-2 mb-2" hidden>
                        <label>Amount (PKR)</label>
