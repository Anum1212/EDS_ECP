        ...
        <div class="card-body">
                ...
                @foreach($voucher->categoryItems($voucher->id,$categoryID) as $item)
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-2 mb-2">
                            ...
                            <input type="text" class="form-control" data-id='225' name="to_{{$itemCount}}" id="to_{{$itemCount}}" {{$item->to}} required>
                        </div>
                        <div class="form-group col-md-1 mb-2">
                            <label>Mode</label>
                            <select class="select2 form-control" name="mode_of_travel_{{$itemCount}}" required>
                                <option {{$item->travel_mode == 'Uber'?'selected':''}} value="Uber">Uber</option>
                                <option {{$item->travel_mode == 'Careem'?'selected':''}} value="Careem">Careem</option>
                                <option {{$item->travel_mode == 'Rikshaw'?'selected':''}} value="Rikshaw">Rikshaw</option>
                                <option {{$item->travel_mode == 'Taxi'?'selected':''}} value="Taxi">Taxi</option>
                                <option {{$item->travel_mode == 'Bus'?'selected':''}} value="Bus">Bus</option>
                                <option {{$item->travel_mode == 'Train'?'selected':''}} value="Train">Train</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-2 mb-2">
                            ...
                            <input type="text" class="form-control" value="{{$item->amount_paid}}" name="amount_paid_{{$itemCount}}" id="amount_paid_{{$itemCount}}" placeholder="PKR" required>
                        </div>
                        <div class="form-group col-md-2 mb-2">
            ...
            @else
                <div class="form-row">
                    ...
                    <div class="form-group col-md-2 mb-2">
                        <label>Amount paid</label>
                        <input type="text" class="form-control" value="0" name="amount_paid_{{$itemCount}}" id="amount_paid_{{$itemCount}}" placeholder="PKR" required>
                    </div>
                    <div class="form-group col-md-2 mb-2">
                        <label>Receipt provided</label>
                        <select class="select2 form-control" name="receipt_provided_{{$itemCount}}" required>
