<div class="card" id="{{$categoryName}}" data-parent="{{$parent}}">
    <div class="card-header card-head-inverse bg-secondary">
        <a data-action="collapse"><h4 class="card-title text-white">Motor Car income tax claim <span class="badge badge bg-white text-secondary badge-pill mr-2"></span></h4></a>
                ...
                @foreach($voucher->categoryItems($voucher->id,$categoryID) as $item)
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4 mb-2">
                            <label>Registration In Name of</label>
                            <select class="select2 form-control" name="vehicle_registration_name_{{$itemCount}}" required>
                                <option {{$item->vehicle_registration_name == 1?'selected':''}} value="Packages Limited">Packages Limited</option>
                                <option {{$item->vehicle_registration_name == 1?'selected':''}} value="Packages Converters Limited">Packages Converters Limited</option>
                                <option {{$item->vehicle_registration_name == 1?'selected':''}} value="DIC Pakistan Limited">DIC Pakistan Limited</option>
                                <option {{$item->vehicle_registration_name == 1?'selected':''}} value="Starch Pack Private Limited">Starch Pack Private Limited</option>
                            </select>
                        </div>
            ...
            @else
                <div class="form-row">
                    ...
                    <div class="form-group col-md-4 mb-2">
                        <label>Registration In Name of</label>
                        <select class="select2 form-control" name="vehicle_registration_name_{{$itemCount}}" required>
                            <option value="Packages Limited">Packages Limited</option>
                            <option value="Packages Converters Limited">Packages Converters Limited</option>
                            <option value="DIC Pakistan Limited">DIC Pakistan Limited</option>
                            <option value="Starch Pack Private Limited">Starch Pack Private Limited</option>
                        </select>
                    </div>
                    <div class="form-group col-md-2 mb-2" hidden>
                        <label>Amount (PKR)</label>
