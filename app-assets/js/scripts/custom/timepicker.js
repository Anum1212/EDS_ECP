...
function updatePicker($input)
{
	...
	let html = '<div class="clock-input' + (clock_enabled ? '' : ' d-none') + '">'
		...
		+ '<div class="invisible w-0"><button type="button" class="btn btn-link input-toggle-btn' + (clock_enabled ? ' d-none' : '') + '" data-input="clock"><i class="far fa-clock fa-fw"></i></button><button type="button" class="btn btn-link input-toggle-btn' + (clock_enabled ? '' : ' d-none') + '" data-input="keyboard"><i class="far fa-keyboard fa-fw"></i></button></div>'
...
jQuery.fn.timepicker = function (options) {
	...
	if (!initialized)
	{
		...
		jQuery(document.head).append('<style id="timepicker-style">'
			...
			+ '.timepicker-popover input { width: 4rem; }'
